...
- [x] pkg/cache/cache.go
  - [x] Cache インターフェース定義（Get, Set, Delete, Clear）
  - [x] CacheMetrics インターフェース定義
- [x] pkg/cache/memorycache/memorycache.go
  - [x] LRU + TTL ベースのインメモリキャッシュ実装
  - [x] 容量制限（MaxMemoryBytes）
  - [x] メトリクス収集（hits, misses, keys, memory）
  - [x] ユニットテスト

#### 11.2 SnapshotManager

- [x] internal/infrastructure/cache/snapshot_manager.go
  - [x] SnapshotManager 構造体
  - [x] GetCurrentToken（現在のスナップショットトークン取得）
  - [x] GenerateWriteToken（書き込み後のトークン生成）
  - [x] PostgreSQL txid_current() を使用した MVCC 対応
  - [x] ユニットテスト

#### 11.3 CheckerWithCache

- [x] internal/services/authorization/checker_with_cache.go
  - [x] CheckerWithCache 構造体（Checker をラップ）
  - [x] キャッシュキー生成（tenant:entity:permission:subject:snapToken）
  - [x] キャッシュ付き Check 処理
  - [x] CheckerInterface 実装

### 12. Closure Table

#### 12.1 マイグレーション

- [x] 000006_add_cursor_pagination_indexes.up.sql
- [x] 000007_create_entity_closure_table.up.sql
  - [x] entity_closure テーブル作成
  - [x] 複合主キー（tenant_id, descendant_type, descendant_id, ancestor_type, ancestor_id）
  - [x] インデックス作成

#### 12.2 RelationRepository 拡張

- [x] updateClosureOnAdd() - Write/BatchWrite 時に Closure Table 自動更新
- [x] updateClosureOnDelete() - Delete/BatchDelete 時に Closure Table 自動更新
- [x] トランザクション内で Closure Table 更新

### 13. メトリクスシステム

#### 13.1 Metrics Collector

- [x] internal/infrastructure/metrics/collector.go
  - [x] Collector 構造体
  - [x] RecordRequest（リクエストカウント）
  - [x] RecordError（エラーカウント）
  - [x] RecordDuration（処理時間）
  - [x] GetAPIMetrics（メトリクス取得）

#### 13.2 Prometheus Exporter

- [x] internal/infrastructure/metrics/prometheus.go
  - [x] PrometheusExporter 構造体
  - [x] grpc_requests_total カウンター
  - [x] grpc_request_duration_seconds ヒストグラム
  - [x] grpc_errors_total カウンター
  - [x] キャッシュメトリクス（hits, misses, hit_rate）

#### 13.3 gRPC Interceptor

- [x] internal/infrastructure/metrics/interceptor.go
  - [x] UnaryServerInterceptor（gRPC インターセプター）
  - [x] リクエスト/エラー/処理時間の自動記録
  - [x] ユニットテスト
...
- [x] cmd/server/main.go 更新
  - [x] CacheConfig 読み込み
  - [x] memorycache 初期化
  - [x] SnapshotManager 初期化
  - [x] CheckerWithCache 初期化
  - [x] Metrics Collector/Exporter 初期化
  - [x] gRPC Interceptor 追加
  - [x] Prometheus HTTP サーバー起動（:9090）
  - [x] グレースフルシャットダウン対応

### 15. リファクタリング後タスク

- [x] Handler 層の SnapToken 統合（DataHandler に TokenGenerator 注入）
- [x] Closure Table 自動更新統合
- [x] SnapshotManager テスト作成
...
2. キャッシュ: LRU + TTL ベースのインメモリキャッシュ（Phase 2 で実装完了）
3. DB 設計: VARCHAR(255)、正規化なし（後で最適化可能）
4. スキーマバージョニング: ULID ベースの自動バージョニング
...
19. キャッシュシステム: LRU + TTL ベースのインメモリキャッシュ
    - SnapshotToken による MVCC 対応（キャッシュキーに含める）
    - PostgreSQL txid_current() でトークン生成
    - TTL フォールバック（デフォルト 5 分）
20. Closure Table: O(1) 祖先検索のための最適化テーブル
    - Write/Delete 時に自動更新
    - 階層的パーミッション評価の高速化
21. メトリクスシステム: Prometheus 形式でメトリクス公開
    - gRPC リクエスト/エラー/処理時間
    - キャッシュヒット率

### 検討中

- なし（Phase 1-2 スコープは確定済み）
...
1. ~~パフォーマンス: キャッシュなしでの性能~~ → Phase 2 で LRU+TTL キャッシュ実装済み
2. ~~LookupAPI: 大量データ時の性能~~ → Closure Table による O(1) 祖先検索で改善

### 残存リスク

- マルチテナント: 現在は固定 tenant_id "default" を使用
- 分散キャッシュ: インメモリキャッシュのため、複数インスタンス間で共有不可
